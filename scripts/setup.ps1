...
if (-not $SkipPull) {
  ...
  docker compose @composeFiles --profile engines pull llamacpp vllm sglang
  if ($LASTEXITCODE -ne 0) {
...
foreach ($id in @(Invoke-Capture docker compose @composeFiles --profile engines ps -q llamacpp vllm sglang airllm imagegen)) {
  if (-not $id) { continue }
  $name = (Invoke-Capture docker inspect -f '{{.Name}}' $id) -replace '^/', ''
  $null = Invoke-Quiet docker rm -f $id
  $staleRemoved += $name
